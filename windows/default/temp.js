...
zindexbase:10,
Winds:0,
init:function(widn, func, height, width, left, top, size, bar){
        ...
        if(core.isMobile) {
        	if(height) {
        		height = (parseInt(window.innerHeight)-50);
        	} else {
	...
	var windowdiv = document.createElement('div');
        if(!bar) {
	var windowdata = '<div id="topbar" class="drag-topbar" style="position:relative;top:0px;left:0;width:100%;height:20px;cursor:pointer;z-index:10;box-shadow:0 0 10px 2px #000;"><div id="left1" class="drag-left1" style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;"><img style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;" src="icons/close-button.png"/></div><div id="left2" class="drag-left2" style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;"><img style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;" src="icons/minimize-button.png"/></div><div id="left3" class="drag-left3" style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;"><img style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;" src="icons/maximize-button.png"/></div></div>';
	var windowdata2 = '<div id="content" class="drag-content" style="position:absolute;top:20px;left:0px;right:0px;bottom:0px;overflow:hidden;background:transparent;z-index:10;box-shadow:0 0 10px 2px #000;"></div>';
        } else if (bar) {
        var windowdata = '<div id="topbar" class="drag-topbar" style="position:relative;top:0px;left:0;width:100%;height:20px;cursor:pointer;z-index:11;background:transparent;box-shadow:0 0 10px 2px #000;"><div id="left1" class="drag-left1" style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;"><img style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;" src="icons/close-button.png"/></div><div id="left2" class="drag-left2" style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;"><img style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;" src="icons/minimize-button.png"/></div><div id="left3" class="drag-left3" style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;"><img style="position:relative;float:left;top:0;left:0;width:14px;height:14px;cursor:pointer;" src="icons/maximize-button.png"/></div></div>';
	var windowdata2 = '<div id="content" class="drag-content" style="position:absolute;top:20px;left:0px;right:0px;bottom:0px;overflow:visible;background:transparent;z-index:10;box-shadow:0 0 10px 2px #000;"></div>';
       }
       if(!size) {
	...
	wid.style.cssText = 'position:fixed;left:0px;top:50px;width:0px;height:0px;display:block;-webkit-border-radius: 30px;-moz-border-radius: 20px;border-radius: 20px;display:none;opacity = 0;-webkit-transform: rotate(360deg);-Moz-transform: rotate(360deg);-webkit-transform : rotateX(360deg);-Moz-transform: rotateX(360deg);';
	if(core.isMobile) {
	...
	wid.style.display = 'block';
        ...
        if(top) {
        wid.style.top = top(wid)<50 ? 50+'px' : top(wid)+'px';
        };
        wid.style.opacity = 1;
        setTimeout(function(){
	...
	wid.topbar.left1.onclick=function(){SimpleWin.close(thisis[actT.x]);};
	wid.topbar.left2.onclick=function(){SimpleWin.minimize(thisis[actT.x]);};
...
create:function(wid, title, data, func, height, width, left, top, size, bar){
	this.zindexbase++;
	this.lastwid = this.lastact;
	var d=SimpleWin;
	wid=this.init(wid, func, height, width, left, top, size, bar);
	window.wid=wid;
...
setfocus:function(wid){
	...
	console.log(wid);
	console.log(thisis[actT.x]);
	//this.zindexbase++;
	/*if(wid != thisis[actT.x]) {
        	this.lastact = thisis[actT.x];
			this.lastwid = thisis[actT.x];
    }*/
	if(this.windows.length > 1) {
		this.windows[this.windows.length-2].style.zIndex -= 1;
	}
...
for(var x=0; x < thisis.length; x++)
{
     if(wid === thisis[x])
     {
        ...
        window.actT.x = x;
        //window.bar(0);
        if(core.UI.taskbar.style.display === 'block') {
        	core.UI.twindow();
        }
        
     }
}
console.log(this.lastwid);
...
close:function(wid){
	...
	if (widclosef){ //if custom event handler function returns true
		...
		if(this.windows.length > 1) {
	        ...
	        this.windows.pop();
	        this.setfocus(this.windows[this.windows.length-1]);
	        
		} else {
			//window.bar(1);
			this.windows.pop();
			if(core.UI.taskbar.style.display == 'none') {
        	core.UI.twindow();
        	}
...
minimize:function(wid){
	...
	if (!wid.min || wid.min === false){
		...
		wid.minimizeI = document.createElement('div');
		wid.minimizeI.className = 'object';
		wid.minimizeI.img = document.createElement('div');
		wid.minimizeI.img.className = 'img';
		wid.minimizeI.img.style.background = 'url(\'../../icons/'+wid.id+'.png\') no-repeat';
		wid.minimizeI.img.style.backgroundSize = '100% 100%';
		wid.minimizeI.font = document.createElement('font');
		wid.minimizeI.font.className = 'font';
		wid.minimizeI.font.innerHTML = wid.id;
		wid.minimizeI.addEventListener('click', function() {
			SimpleWin.minimize(wid);
			}, false);
		/*<div class="object">
          <div class="img">
          </div>
		...
		core.UI.dock.children[0].children[0].appendChild(wid.minimizeI);
		wid.minimizeI.appendChild(wid.minimizeI.img);
		wid.minimizeI.appendChild(wid.minimizeI.font);
		wid.min=true;
		if(this.windows.length > 1) {
	        //this.setfocus(this.lastwid);
	        this.setfocus(this.windows[this.windows.length-2]);
		} else {
			//window.bar(1);
			if(core.UI.taskbar.style.display == 'none') {
        	core.UI.twindow();
                ...
                core.UI.dock.children[0].children[0].removeChild(wid.minimizeI);
                wid.nim-=1;
		wid.style.display='block';
		wid.style['-webkit-transition'] = 'all .2s';
		wid.style.MozTransition = 'all .2s';
...
maximize:function(wid){
	if (!wid.max || wid.max === false){
		...
		wid.style.left=0+'px';
		wid.style.width=100+'%';
		wid.style.height=(parseInt(window.innerHeight)-50)+'px';
		wid.max=true;
